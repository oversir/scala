...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should return the correct value on an interior pixel " +
    "of a 9x11 image with radius 3") {
    val src = new Img(9, 11)
    for (y <- 0 until 11; x <- 0 until 9)
      src(x, y) = y*11 + x

    assert(boxBlurKernel(src, 5, 4, 3) === 49,
      s"(boxBlurKernel(5, 4, 3) should be 49, " +
        s"but it's ${boxBlurKernel(src, 5, 4, 3)})")
  }
