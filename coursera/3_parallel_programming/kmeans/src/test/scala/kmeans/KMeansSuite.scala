...
class KMeansSuite extends FunSuite {
  ...
  test("'converged' should succeed or fail for various eta's") {
    val p1 = new Point(1, 1, 0)
    val p2 = new Point(1, -1, 0)
    val p3 = new Point(-1, 1, 0)
    val p41 = new Point(-1, -1, 0)
    val p42 = new Point(-1, -1, 0.1)
    val previous: GenSeq[Point] = IndexedSeq(p1, p2, p3, p41)
    val current: GenSeq[Point] = IndexedSeq(p1, p2, p3, p42)
    val etaLarge = 1.0
    val etaSmall = Double.MinValue
    assert(converged(etaLarge)(previous, current),
           s"converged($etaLarge)($previous, $current) should equal to true")
    assert(!converged(etaSmall)(previous, current),
           s"converged($etaSmall)($previous, $current) should equal to false")
  }
